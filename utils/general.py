...
class FVbinder:
    ...
    @staticmethod
    def get_mask_from_sequence(sequence: str) -> torch.Tensor:
        mask = torch.zeros((len(sequence), 5))
        a = Chain(sequence, scheme="chothia")
        for reg, positions in a.regions.items():
            if reg.startswith("FR"):
                continue
            for pos, amino in positions.items():
                mask[pos.number] = torch.ones(5)
        return mask

    def to_pdb(self, filename: str):
        assert self.coords is not None
        seqs = [self.vh, self.vl]
        full_seq = "".join(seqs)
        chains = ["H", "L"]
...
class CustomIgFold:
    ...
    def fold(self, binder: FVbinder):
        ...
        seqs = [" ".join(list(i)) for i in [binder.vh, binder.vl]]
